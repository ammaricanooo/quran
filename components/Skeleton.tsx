  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex md:justify-end">
        <div className="w-full h-12 bg-white/10 rounded-4xl animate-pulse" />
      </div>
      {Array.from({ length: count }).map((_, i) => (
        ...
        >
          ...
          <div className="flex justify-between items-center">
            <div className="h-5 w-20 bg-white/10 rounded-xl animate-pulse" />
            <div className="w-8 h-8 bg-white/10 rounded-xl animate-pulse" />
          </div>
          {/* Title */}
          <div className="h-6 w-3/4 bg-white/10 rounded-xl animate-pulse" />
          {/* Arabic text */}
          <div className="space-y-2">
            <div className="h-8 w-full bg-white/10 rounded-xl animate-pulse" />
            <div className="h-8 w-4/5 bg-white/10 rounded-xl animate-pulse ml-auto" />
          </div>
          {/* Translation */}
          <div className="space-y-2 pl-4 border-l-2 border-white/5">
            <div className="h-4 w-full bg-white/10 rounded-xl animate-pulse" />
            <div className="h-4 w-5/6 bg-white/10 rounded-xl animate-pulse" />
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex md:justify-end mb-6">
        <div className="w-full md:w-64 h-11 bg-white/10 rounded-xl animate-pulse" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {Array.from({ length: count }).map((_, i) => (
          <div
            key={i}
            className="bg-white/5 border border-white/5 p-5 rounded-4xl space-y-4"
          ...
          >
            ...
            <div className="h-5 w-2/3 bg-white/10 rounded-xl animate-pulse" />
      ...
      {Array.from({ length: count }).map((_, i) => (
        ...
        >
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="w-7 h-7 bg-white/10 rounded-xl animate-pulse" />
          </div>
          {/* Arabic */}
          <div className="space-y-2">
          ...
          {/* Translation */}
          <div className="h-4 w-3/4 bg-white/10 rounded animate-pulse pl-4 border-l border-white/5" />
          {/* Progress bar */}
...
export function SkeletonJadwal() {
  return (
    <div className="max-w-5xl mx-auto flex flex-col gap-6">
      {/* Desert BG area */}
      <div className="h-36 md:h-44 bg-white/10 rounded-3xl animate-pulse" />
...
export function SkeletonAyatList({ count = 5 }: { count?: number }) {
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      {/* Surah info card */}
      <div className="h-44 bg-white/10 rounded-4xl animate-pulse mb-4" />
      ...
      <div className="mb-10">
        ...
        <div className="flex gap-4 overflow-hidden">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="min-w-[9rem] h-28 bg-white/10 rounded-3xl animate-pulse shrink-0" />
          ))}
        </div>
      </div>
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
            ...
            <div className="space-y-2 flex-1">
              <div className="h-8 w-full bg-white/10 rounded-xl animate-pulse" />
              <div className="h-8 w-5/6 bg-white/10 rounded-xl animate-pulse ml-auto" />
            </div>
          </div>
          {/* Latin + Terjemahan */}
          <div className="space-y-2 pl-4 border-l-2 border-white/5">
...
// ─── Maulid list grid (2-col, matches card: icon + badge + title) ─────────────
export function SkeletonMaulidGrid({ count = 6 }: { count?: number }) {
  return (
    <div className="grid gap-5 md:grid-cols-2">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="rounded-4xl border border-white/5 bg-white/5 p-6 space-y-5"
          style={{ animationDelay: `${i * 60}ms` }}
        >
          {/* Icon + badge row */}
          <div className="flex items-start justify-between gap-4">
            <div className="w-11 h-11 bg-white/10 rounded-2xl animate-pulse shrink-0" />
            <div className="h-5 w-24 bg-white/10 rounded-xl animate-pulse" />
          </div>
          {/* Title */}
          <div className="h-6 w-3/4 bg-white/10 rounded-xl animate-pulse" />
        </div>
...
// ─── Maulid reading detail (matches reading card: badge+title+arabic+latin) ───
export function SkeletonMaulidReadings({ count = 4 }: { count?: number }) {
  return (
    <div className="space-y-5">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="rounded-4xl border border-white/5 bg-white/5 p-5 md:p-7 space-y-5"
          style={{ animationDelay: `${i * 80}ms` }}
        >
          {/* Number badge + section title */}
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white/10 rounded-2xl animate-pulse shrink-0" />
            <div className="h-4 w-48 bg-white/10 rounded-xl animate-pulse" />
          </div>
          {/* Arabic text block (right-aligned to mimic RTL) */}
          <div className="space-y-3">
            <div className="h-8 w-full bg-white/10 rounded-xl animate-pulse" />
            <div className="h-8 w-4/5 bg-white/10 rounded-xl animate-pulse ml-auto" />
            <div className="h-8 w-3/4 bg-white/10 rounded-xl animate-pulse ml-auto" />
          </div>
          {/* Transliteration */}
...
// ─── Article card grid (2-col, image top + text body) ────────────────────────
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2">
      {Array.from({ length: count }).map((_, i) => (
        <div
          key={i}
          className="overflow-hidden rounded-4xl border border-white/5 bg-white/5"
          style={{ animationDelay: `${i * 60}ms` }}
        >
          {/* Thumbnail */}
          ...
          <div className="p-5 space-y-3">
            ...
            <div className="space-y-2">
              <div className="h-5 w-full bg-white/10 rounded-xl animate-pulse" />
              <div className="h-5 w-4/5 bg-white/10 rounded-xl animate-pulse" />
            </div>
